...
class PiecePlacement(private val airCraftCarrier: String, private val battleship: String,
    ...
    override fun call(): SignedTransaction {
        ...
        val playerStatesFromVault = serviceHub.vaultService.queryBy<PlayerState>(criteria).states
        ...
        val gameBoard = GameContract.Board()
        for(coordinate in coordinates) {
            gameBoard.board[coordinate.first][coordinate.second] = 1
        }

        val playerState = if(playerStatesFromVault.isEmpty()) {
            PlayerState(gameBoard, listOf(ourIdentity))
        } else {
        ...
        val transactionBuilder: TransactionBuilder = TransactionBuilder(notary)
                ...
                .addCommand(GameContract.Commands.AddGamePiece(), listOf(ourIdentity.owningKey))
